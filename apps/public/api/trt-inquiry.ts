...
const TRT_RECIPIENTS = (
  process.env.EMAIL_RECIPIENTS_TRT ||
...
function generateStaffEmailHtml(data: TRTPayload): string {
  const isEnrollment = data.interest === "enroll";
  const badgeLabel = isEnrollment ? "TRT ENROLLMENT REQUEST" : "TRT INFORMATION REQUEST";
...
<body>
  <div class="container">
    <div class="header">
      <h1>Men's Health & TRT Inquiry</h1>
      <p>Testosterone Replacement Therapy — HCI Medical Group</p>
    </div>
    <div class="content">
    ...
    <div class="footer">
      <p>Submitted through the Men's Health & TRT page on hcimed.com</p>
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p>Men's Health & TRT Program</p>
    </div>
    <div class="content">
      <h2>Thank you, ${escapeHtml(firstName)}!</h2>
      <p>We've received your ${isEnrollment ? "enrollment request for our Men's Health & TRT Program" : "request for information about our Men's Health & TRT Program"}. A member of our team will contact you within 1 business day.</p>
      ${isEnrollment ? `<p>Here's what to expect at your initial evaluation:</p>
      <div class="info-box">
        <p><strong>Duration:</strong> 30\u201345 minutes</p>
        <p><strong>Includes:</strong> Comprehensive health evaluation, symptom assessment, physical exam, and lab orders</p>
        <p><strong>Cost:</strong> $349 for the initial evaluation</p>
        <p><strong>Lab note:</strong> Two separate morning fasting blood draws (before 10 AM) are required for diagnosis</p>
        <p><strong>Location:</strong> 65 N. Madison Ave. #709, Pasadena, CA 91101</p>
      </div>` : ""}
      <p>If you have any questions, please call us at <a href="tel:6267924185" style="color:#2d8bc9;font-weight:600;">(626) 792-4185</a>.</p>
...
export async function POST(request: Request) {
  try {
    ...
    const data: TRTPayload = await request.json();
    ...
    const isEnrollment = data.interest === "enroll";
    const subjectPrefix = isEnrollment ? "TRT Enrollment" : "TRT Inquiry";

    const { error } = await resend.emails.send({
      from: "HCI Men's Health Program <noreply@hcimed.com>",
      to: TRT_RECIPIENTS,
      cc: ["admin@hcimed.com"],
      replyTo: data.email,
    ...
    try {
      ...
      console.error("Failed to send TRT confirmation email:", confirmError);
    }

...
import { awvUploadRowSchema, validateUploadHeaders, type AwvUploadRow } from '@/schemas/awvUploadSchema';
import { UPLOAD_REQUIRED_COLUMNS, UPLOAD_OPTIONAL_COLUMNS } from '@/lib/awv-tracker-constants';
import { useAwvUpload } from '@/hooks/useAwvUpload';
...
const ALL_COLUMNS = [...UPLOAD_REQUIRED_COLUMNS, ...UPLOAD_OPTIONAL_COLUMNS];
...
export function UploadTab() {
  ...
  const [parsedRows, setParsedRows] = useState<AwvUploadRow[]>([]);
  const [validCount, setValidCount] = useState(0);
  const [invalidCount, setInvalidCount] = useState(0);
  const [rawPreview, setRawPreview] = useState<Record<string, string>[]>([]);
  const [uploadResult, setUploadResult] = useState<MockUploadResult | null>(null);
  const [errorMessage, setErrorMessage] = useState('');

  const uploadMutation = useAwvUpload();
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        try {
          ...
          const validation = validateUploadHeaders(headers);
          ...
          // Validate each row
          const validRows: AwvUploadRow[] = [];
          let invalid = 0;

          for (const row of results.data as Record<string, string>[]) {
            const result = awvUploadRowSchema.safeParse(row);
            if (result.success) {
  ...
  // Preview table columns — show recognized columns first
